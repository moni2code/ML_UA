...
def train_one_epoch(model, loss, optimizer, ds):
  '''
...
def train(model, loss, val_metrics, optimizer, train_ds, dev_ds, num_epochs=10, record_weights=None):

  '''
  Train a PyTorch model over multiple epochs and record training metrics.
  ...
  This function trains a PyTorch model over multiple epochs using the provided
  dataset, loss function, optimizer, and validation metrics. It records and 
  returns the training history, including training loss and validation metrics
  at each epoch.
